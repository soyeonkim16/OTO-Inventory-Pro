...
export default async (request) => {
  if (request.method !== 'POST') return json(405, { error: 'POST 요청만 허용됩니다.' });
  if (!SUPABASE_URL || !SUPABASE_SECRET_KEY) {
    return json(500, { error: 'Netlify 환경변수 SUPABASE_URL 또는 SUPABASE_SECRET_KEY가 없습니다.' });
  }
  ...
  if (authError || !authData?.user) return json(401, { error: '로그인 정보가 유효하지 않습니다.' });
  ...
  if (requesterError || !requester || requester.role !== 'admin' || !requester.active) {
    return json(403, { error: '관리자만 직원 계정을 관리할 수 있습니다.' });
  }
  ...
  try {
    switch (body.action) {
      case 'list': {
        const { data: userPage, error } = await admin.auth.admin.listUsers({ page: 1, perPage: 1000 });
        ...
        const users = userPage.users
          .map(user => {
            ...
            return {
              ...
              login_id: email.endsWith('@login.otolab.co.kr') ? email.replace('@login.otolab.co.kr', '') : email,
          ...
          .sort((a, b) => (a.role === b.role ? a.name.localeCompare(b.name, 'ko') : a.role === 'admin' ? -1 : 1));
      ...
      case 'create': {
        ...
        if (password.length < 6) return json(400, { error: '비밀번호는 최소 6자 이상이어야 합니다.' });
    ...
    const friendly = message.toLowerCase().includes('already been registered')
      ? '이미 사용 중인 아이디입니다.'
      : message;
