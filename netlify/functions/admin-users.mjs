...
export default async (request) => {
  ...
  if (
    ...
    return json(500, {
      error:
        'Netlify 환경변수 SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY 또는 SUPABASE_SECRET_KEY가 없습니다.'
    });
  }
  ...
  if (authError || !authData?.user) {
    ...
    return json(401, {
      error: '로그인 정보가 유효하지 않습니다. 로그아웃 후 다시 로그인해주세요.'
    });
  }
  ...
  if (requesterError) {
    return json(500, {
      error: `관리자 권한 확인 실패: ${requesterError.message}`
    });
  }

  if (!requester || requester.role !== 'admin' || !requester.active) {
    return json(403, {
      error: '활성화된 관리자만 직원 계정을 관리할 수 있습니다.'
    });
  }
  ...
  try {
    switch (body.action) {
      ...
      case 'create': {
        ...
        const loginId = String(employee.login_id || '')
          .trim()
          .toLowerCase();
      ...
      case 'set_role': {
        ...
        if (userId === requesterId && role !== 'admin') {
          return json(400, {
            error: '현재 로그인한 자신의 관리자 권한은 해제할 수 없습니다.'
          });
        }
      ...
      case 'set_active': {
        ...
        if (userId === requesterId && !active) {
          return json(400, {
            error: '현재 로그인한 자신의 계정은 중지할 수 없습니다.'
          });
        }
      ...
      case 'delete_user': {
        ...
        if (userId === requesterId) {
          return json(400, {
            error: '현재 로그인한 자신의 계정은 삭제할 수 없습니다.'
          });
        }
    ...
    if (lower.includes('invalid api key')) {
      return json(500, {
        error: 'Supabase API 키가 올바르지 않습니다.'
      });
    }
